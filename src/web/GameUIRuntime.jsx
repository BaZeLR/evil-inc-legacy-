...
import { startGame } from '../core/bootstrap/startGame.js';
...
import { getMindReadThoughtsForEntity } from '../utils/mindReadCatalog.js';
...
import { handleTalk, handleAsk, runUseAction, runGiveAction, runBuyAction, runAbilityAction, handleAbility, isAbilityLabel } from '../actions/index.js';
import { normalizeTextList, createUiLogEntry, DEFAULT_MEDIA_TIMING, resolveMediaUrl, isVideoMedia } from '../ui/formatting/index.js';
import { SPEAKER_LABELS } from '../ui/theme/speakers.js';
import { ComUnitModules } from './components/uicomponents/ComUnitModules/ComUnitModules.jsx';
import { StoryLog } from './components/StoryLog.jsx';
import { OverlayMediaPanel } from './components/OverlayMediaPanel.jsx';
import { VendorDrawer } from './components/drawers/VendorDrawer.jsx';
import { CombatDrawer } from './components/drawers/CombatDrawer.jsx';
import { CombatOverlay, CombatLog } from './components/CombatOverlay.jsx';
import { StartScreen } from './components/StartScreen.jsx';
import {
  handleToggleContainerOpen,
  handleToggleEquipped,
  handleTooltipTakeFromContainer,
  handleTooltipToggleContainer,
  handleTooltipEquip,
  handleTooltipUnequip,
  handleMoveClick
...
export function GameUIRuntime({ startRequest, onStartRequestHandled, onRequestStartFlow } = {}) {
  const {
    game,
    setGame,
    currentRoom,
  ...
  const [telepathyPickerOpen, setTelepathyPickerOpen] = useState(false);
  ...
  const [storyLogResetToken, setStoryLogResetToken] = useState(0);
  ...
  useEffect(() => {
    ...
    setTelepathyPickerOpen(false);
  }, [inspectTarget?.type, inspectTarget?.id]);

  useEffect(() => {
    if (activeDrawer === 'actions') return;
    if (!telepathyPickerOpen) return;
    setTelepathyPickerOpen(false);
  }, [activeDrawer, telepathyPickerOpen]);
  ...
  const showPlayerPortraitOverride = (media, { durationMs = DEFAULT_MEDIA_TIMING.portrait } = {}) => {
    if (!media) return;
    if (playerPortraitTimeoutRef.current) {
      clearTimeout(playerPortraitTimeoutRef.current);
      playerPortraitTimeoutRef.current = null;
  ...
  const showTransientMedia = (media, { title = null, durationMs = DEFAULT_MEDIA_TIMING.transient } = {}) => {
    clearTransientMedia();
  ...
  const getMindReadThoughts = entity => {
    if (!entity) return [];
    const isReadable =
      isTruthyFlag(entity?.isMindreadable) ||
      isTruthyFlag(entity?.IsMindreadable) ||
      isTruthyFlag(entity?.Mindreadable) ||
      isTruthyFlag(entity?.mindReadable) ||
      isTruthyFlag(entity?.MindReadable) ||
      isTruthyFlag(entity?.mindRead) ||
      isTruthyFlag(entity?.MindRead) ||
      isTruthyFlag(getCustomProperty(entity, 'Mindreadable')) ||
      isTruthyFlag(getCustomProperty(entity, 'mindreadable')) ||
      isTruthyFlag(getCustomProperty(entity, 'mindReadable')) ||
      isTruthyFlag(getCustomProperty(entity, 'MindReadable')) ||
      isTruthyFlag(getCustomProperty(entity, 'mindRead')) ||
      isTruthyFlag(getCustomProperty(entity, 'MindRead'));
    if (!isReadable) return [];

    const fileThoughts = getMindReadThoughtsForEntity(entity);
    if (Array.isArray(fileThoughts) && fileThoughts.length) {
      return fileThoughts.map(entry => String(entry ?? '').trim()).filter(Boolean);
    }
    ...
    const thoughtsRaw =
      ...
      entity?.mindReadThoughts ??
      getCustomProperty(entity, 'Thoughts') ??
      getCustomProperty(entity, 'thoughts') ??
  ...
  const beginIntroSequence = (gameOverride = game) => {
    ...
    setActiveDrawer(null);
    setDismissedOverlayKey(null);
    setStoryLogResetToken(prev => prev + 1);
    clearContinuePrompt(g);
  ...
  const getCustomProperty = (obj, propertyName) => {
    ...
    const match = props.find(entry => {
      if (!entry || typeof entry !== 'object') return false;
      const propName = String(entry?.Property ?? '').trim().toLowerCase();
      if (propName === lowerKey) return true;
      return Object.keys(entry).some(raw => String(raw ?? '').trim().toLowerCase() === lowerKey);
    });
    if (!match) return undefined;
    if (String(match?.Property ?? '').trim().toLowerCase() === lowerKey) return match.Value;
    const directKey = Object.keys(match).find(raw => String(raw ?? '').trim().toLowerCase() === lowerKey);
    return directKey ? match[directKey] : undefined;
  };
  ...
  const takeObject = obj => {
    ...
    for (const [rid, mappedRoom] of Object.entries(game.roomMap || {})) {
      if (!mappedRoom || !Array.isArray(mappedRoom.objects)) continue;
      const before = mappedRoom.objects.length;
      mappedRoom.objects = mappedRoom.objects.filter(entry => (entry?.id ?? entry?.UniqueID) !== objId);
      if (mappedRoom.objects.length !== before) changedRoomIds.push(String(rid));
    }
    ...
    const save = ensureSaveGameShape(game);
    commitPlayerToSave(save);
    const uniqueChangedRoomIds = [...new Set(changedRoomIds)];
    for (const changedRoomId of uniqueChangedRoomIds) {
      commitRoomToSave(changedRoomId, save);
    }
    persistSaveGame(save);
  };
  ...
  const isComUnitInventoryItem = item => {
    const id = String(item?.id ?? '').trim().toLowerCase();
    const objId = String(item?.obj?.UniqueID ?? item?.obj?.id ?? '').trim().toLowerCase();
    const name = String(item?.name ?? item?.label ?? '').trim().toLowerCase();
    if (!id && !objId && !name) return false;
    if (id === 'comunit_001' || objId === 'comunit_001') return true;
    if (id.includes('comunit') || objId.includes('comunit')) return true;
    if (name === 'com unit' || name.includes('com unit')) return true;
    return false;
  };
  ...
  const resolvedRoomObjects = (roomObjects || [])
    ...
    .filter(obj => {
      const objId = String(obj?.id ?? obj?.UniqueID ?? '').trim();
      if (!objId) return false;
      return !hasInventoryItem(objId);
    })
    .filter(obj =>
  ...
  const runEntityActionEvent = ({ entityType, entityId, entity, eventType, label, input } = {}) => {
    ...
    if (entityType === 'character' && isMindReadAction(name)) {
      ...
      result.texts = [thought ? `&lt;${thought}&gt;` : 'You reach for their mind, but only static comes back.'];
      if (!result.media) {
  ...
  const buildActionsDrawerModel = () => {
    ...
    if (telepathyPickerOpen) {
      const targets = (roomNpcs || [])
        .map(npc => {
          const id = String(npc?.id ?? npc?.UniqueID ?? '').trim();
          if (!id) return null;
          const resolvedNpc = game?.characterMap?.[id] ?? npc;
          const rawName = String(
            resolvedNpc?.Charname ??
              resolvedNpc?.Name ??
              resolvedNpc?.name ??
              npc?.Charname ??
              npc?.Name ??
          ...
          const name = rawName || (isUuidLike(id) ? 'Unknown Character' : humanizeId(id) || SPEAKER_LABELS.npc);
          const desc = resolvedNpc?.Description || resolvedNpc?.description || npc?.Description || npc?.description || `Read ${name}'s thoughts.`;
          return { id, name, desc, npc: resolvedNpc };
        })
        .filter(Boolean);

      const items = targets.length
        ? targets.map(target => ({
            id: `telepathy-target:${target.id}`,
            label: target.name,
            description: target.desc,
            onClick: () => {
              setInspectTarget({ type: 'npc', id: target.id });
              runEntityActionEvent({
                entityType: 'character',
                entityId: target.id,
                entity: target.npc,
                eventType: 'Telepathy',
                label: `Telepathy: ${target.name}`
              });
            }
          }))
        : [
            {
              id: 'telepathy-target:none',
              label: 'No character minds nearby',
              description: 'Move to a room with characters and try again.',
              disabled: true
            }
          ];
      ...
      return {
        title: "Who's mind?",
        description: 'Select a nearby character.',
        sections: [{ title: 'Targets', items }]
      };
    }

    if (inspectTarget.type === 'npc' && inspectedNpc) {
      const npcId = inspectedNpc?.id ?? inspectedNpc?.UniqueID ?? null;
      const npcName = inspectedNpc?.Charname || inspectedNpc?.name || inspectedNpc?.Name || humanizeId(npcId) || SPEAKER_LABELS.npc;
        ...
        if (actionKey.startsWith('buy ')) {
          ...
          if (match) {
            items.push({
              ...
              description: entry.description,
              disabled: !canShopInspectedObject,
              onClick: () => runBuyAction({ buyVendorItem, vendorId: objId, itemId: match.itemId, price: match.price })
            });
            continue;
          }
  ...
  const runCombatAction = action => {
    ...
    if (abilityPortrait) {
      showPlayerPortraitOverride(abilityPortrait, { durationMs: DEFAULT_MEDIA_TIMING.combatPortrait });
    }
  ...
  return (
    <div className="main-container">
      {startVariantPromptOpen ? (
        <StartScreen
          onPlayIntro={handleStartWizardIntro}
          onSkipIntro={() => handleStartVariantChoice('skip')}
          onContinue={closeStartVariantPrompt}
        />
      ) : null}
      {textInputPrompt ? (
        <div
        ...
        <div className="left-panel-drawer" aria-hidden={!activeDrawer}>
          {activeDrawer && (
            <>
                      ...
                      <div className="inventory-action-row">
                        {isContainerObject(selectedInventoryItem.obj) ? (
                          <button
                            type="button"
                            className="drawer-action-btn"
                            onClick={() => handleToggleContainerOpen({ toggleContainerOpen, id: selectedInventoryItem.id })}
                          >
                          ...
                          <button
                            type="button"
                            className="drawer-action-btn"
                            onClick={() => handleToggleEquipped({ toggleEquipped, id: selectedInventoryItem.id })}
                        ...
                        {isComUnitInventoryItem(selectedInventoryItem) ? (
                          <ComUnitModules
                            item={selectedInventoryItem}
                            game={game}
                            room={currentRoom}
                            shouldShowMenuEntry={shouldShowMenuEntry}
                            hackTargets={resolvedRoomObjects
                              .map(obj => {
                                const id = String(obj?.id ?? obj?.UniqueID ?? '').trim();
                                if (!id) return null;
                                const entity = game?.objectMap?.[id] ?? obj;
                                ...
                                const desc = entity?.Description || entity?.description || null;
                                const media = entity?.Picture || entity?.media || null;
                                return { id, name, desc, media, entity };
                              })
                              .filter(Boolean)}
                            onRunAction={(actionLabel, input = null) => {
                              const obj = selectedInventoryItem?.obj ?? null;
                              const objId = String(selectedInventoryItem?.id ?? '').trim();
                              if (!obj || !objId) return;
                              runEntityActionEvent({
                                entityType: 'object',
                                entityId: objId,
                                ...
                                label: `${selectedInventoryItem.name}: ${actionLabel}`,
                            ...
                            onHackTarget={target => {
                              ...
                              if (!targetId) return;
                              const targetEntity = target?.entity ?? game?.objectMap?.[targetId] ?? null;
                              if (!targetEntity) return;
                              const targetName = target?.name || targetEntity?.Name || targetEntity?.name || humanizeId(targetId) || targetId;
                              const menuEntry = findActionMenuEntry(targetEntity, 'Hack');
                              const ctx = {
                                game,
                                room: currentRoom,
                                vars: game?.variables ?? {},
                                entity: targetEntity,
                                objectBeingActedUpon: targetEntity
                              };
                              const placeholder =
                              ...
                              openTextInputPrompt({
                                title: `${targetName}: Hack`,
                                placeholder,
                                submitLabel,
                                media: target?.media || targetEntity?.Picture || targetEntity?.media || null,
                                inputType: menuEntry?.MaskInput ? 'password' : 'password',
                                entityType: 'object',
                                entityId: targetId,
                                entity: targetEntity,
                                eventType: 'Hack',
                                label: `${targetName}: Hack`
                              });
                            }}
                          />
                        ) : null}
                        {(() => {
                          if (isComUnitInventoryItem(selectedInventoryItem)) return null;
                          if (combat) return null;
                          const obj = selectedInventoryItem.obj;
                                        ...
                                        <div className="container-tooltip" role="menu" aria-label={`${contentName} actions`}>
                                          ...
                                          {!contentInInventory ? (
                                            <button
                                              ...
                                              className="container-tooltip-btn"
                                              onClick={event =>
                                                handleTooltipTakeFromContainer(event, {
                                                  takeFromContainerToInventory,
                                                  containerId,
                                                  contentId
                                                })
                                              }
                                          ...
                                          {contentIsContainer ? (
                                            <button
                                              ...
                                              className="container-tooltip-btn"
                                              onClick={event =>
                                                handleTooltipToggleContainer(event, {
                                                  setHoveredContainerItemId,
                                                  contentId,
                                                  toggleContainerOpen
                                                })
                                          ...
                                          {contentEquipable ? (
                                            <>
                                              <button
                                                ...
                                                onClick={event =>
                                                  handleTooltipEquip(event, {
                                                    setHoveredContainerItemId,
                                                    contentId,
                                                    contentInInventory,
                                                    setItemEquipped,
                                                    getActionsMenuDescription,
                                                    contentObj,
                                                    setEventMedia,
                                                    setEventMediaTitle,
                                                    contentName,
              ...
              {activeDrawer === 'vendor' && (
                <VendorDrawer
                  shopVendorId={shopVendorId}
                  roomObjects={roomObjects}
                  roomNpcs={roomNpcs}
                  isVendorObject={isVendorObject}
                  resolveEntityId={resolveEntityId}
                  game={game}
                  humanizeId={humanizeId}
                  resolveVendorShopItems={resolveVendorShopItems}
                  toSafeInt={toSafeInt}
                  isWeaponObject={isWeaponObject}
                  shopCategory={shopCategory}
                  setShopCategory={setShopCategory}
                  credits={credits}
                  getInventoryQuantity={getInventoryQuantity}
                  hasInventoryItem={hasInventoryItem}
                  shopHoveredItemId={shopHoveredItemId}
                  setShopHoveredItemId={setShopHoveredItemId}
                  runBuyAction={runBuyAction}
                  buyVendorItem={buyVendorItem}
                />
              )}
              ...
              {activeDrawer === 'combat' && combat && (
                <CombatDrawer
                  ...
                  runCombatAction={runCombatAction}
                      ...
                      visibleExits.map((exit, idx) => (
                        <li key={`${exit.direction}:${exit.destinationId || exit.destinationRaw || idx}`}>
                          <button
                            ...
                            onClick={() => handleMoveClick({ handleMove, destinationId: exit.destinationId })}
      ...
      <main className="main-area">
        <section className="upper-split">
          ...
          <OverlayMediaPanel
            showOverlayMediaPanel={showOverlayMediaPanel}
            overlayMediaUrl={overlayMediaUrl}
            shopHoveredItemId={shopHoveredItemId}
            shopPurchaseNotice={shopPurchaseNotice}
            activeDrawer={activeDrawer}
            overlayKey={overlayKey}
            setDismissedOverlayKey={setDismissedOverlayKey}
          ...
          <CombatOverlay
            ...
            openCombatMenu={openCombatMenu}
            runCombatAction={runCombatAction}
            playerHpPercent={playerHpPercent}
            playerEnergyPercent={playerEnergyPercent}
            enemyHpPercent={enemyHpPercent}
            enemyEnergyPercent={enemyEnergyPercent}
            playerPortraitUrl={playerPortraitUrl}
            resolveMediaUrl={resolveMediaUrl}
            showCombatFlash={showCombatFlash}
            combatEffects={combatEffects}
            restartGame={restartGame}
            exitCombat={exitCombat}
            showCombatRewards={showCombatRewards}
                  ...
                  <div className="side-text-lines">
                    {introActive ? (
                      ...
                      <StoryLog
                        ...
                        ragsToHtml={ragsToHtml}
                        scrollContainerRef={sideTextWindowRef}
                        resetSignal={storyLogResetToken}
                      />
                    )}
                  </div>
                </>
        ...
        <BottomStatusPanel
          ...
          onShowAbility={ability => {
            ...
            runAbilityAction({
              ...
              onTelepathy: () => {
                setHackPickerOpen(false);
                setTelepathyPickerOpen(true);
                setActiveDrawer('actions');
                setEventMedia(null);
                setEventMediaTitle(null);
                setEventMessages(["<b>Telepathy</b>", "Who's mind?"]);
              },
              onDefault: (name, entry) => {
                const tip = String(entry?.Tooltip ?? entry?.tooltip ?? '').trim();
                setEventMedia(null);
                setEventMediaTitle(null);
          ...
          onInventoryAction={({ item, action }) => {
            ...
            if (actionKey === 'use') {
              runUseAction({
                ...
                labelPrefix: `${item?.name || item?.label || humanizeId(objId) || objId}`
              });
              return;
            }
          ...
          onNpcAction={({ npc, npcId, action }) => {
            ...
            if (!label) return;
            const actionKey = normalizeActionName(label);
            const npcName = npc?.Charname || npc?.Name || npc?.name || humanizeId(id) || SPEAKER_LABELS.npc;
          ...
          onTalk={npc => {
            ...
            return handleTalk({
              ...
              entityId: npcId,
              entity: npc,
              labelPrefix: npc?.Charname || npc?.Name || npc?.name || humanizeId(npcId) || SPEAKER_LABELS.npc,
              eventType: 'Talk'
            });
          }}
